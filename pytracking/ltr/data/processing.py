...
class DiMPProcessing(BaseProcessing):
    ...
    def __call__(self, data: TensorDict):
        ...
        """
        cv2.imwrite('/home/robotics-meta/Desktop/Jason/pytracking/train_image.png',(data['train_images'][0]))
...
class DiMPMergedProcessing(BaseProcessing):
    """ The processing class used for training DiMP. The images are processed in the following way.
    First, the target bounding box is jittered by adding some noise. Next, a square region (called search region )
    centered at the jittered target center, and of area search_area_factor^2 times the area of the jittered box is
    cropped from the image. The reason for jittering the target box is to avoid learning the bias that the target is
    ...
    def __call__(self, data: TensorDict):
        ...
        #cv2.imwrite('/home/robotics-meta/Desktop/Jason/pytracking/train_mask.png',(data['train_masks'][0].numpy()*255).astype(np.uint8))
        ...
        for s in ['train', 'test']:
            ...
            #np_masks=[ten.numpy() for ten in data['train_masks']]
            
            
            '''crops, boxes,masks = prutils.target_image_crop(data[s + '_images'], jittered_anno, data[s + '_anno'],
                                                     self.search_area_factor, self.output_sz, mode=self.crop_type,
                                                     max_scale_change=self.max_scale_change,masks=data[s + '_masks'])'''
            ...
            data[s + '_images'], data[s + '_anno'],data[s + '_masks'] = self.transform[s](image=crops, bbox=boxes,mask=masks, joint=False)
        ...
        cv2.imwrite('/home/robotics-meta/Desktop/Jason/pytracking/process.png',(data['train_images'][0].permute(1, 2, 0).numpy()*255).astype(np.uint8))
