...
from ltr.dataset.vos_base import VOSDatasetBase, VOSMeta,MergedDatasetBase
from pytracking.evaluation import Sequence
from ltr.admin.environment import env_settings
from ltr.data.image_loader import jpeg4py_loader


class Vessel_seg(MergedDatasetBase):
    ...
    def __init__(self, root=None, sequences=None, version='2017', split='train', multiobj=True,
        ...
        if version == '2017':
            if split in ['train', 'val']:
                root = env_settings().vessel_seg if root is None else root
        ...
        self._boxpath=dset_path / 'BBoxAnnos'

        meta_path = dset_path / "generated_meta.json"
        if meta_path.exists():
            self.gmeta = VOSMeta(filename=meta_path)
        else:
            self.gmeta = VOSMeta.generate('vessel_seg', self._jpeg_path, self._anno_path)
