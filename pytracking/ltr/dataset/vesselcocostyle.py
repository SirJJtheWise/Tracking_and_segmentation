...
class Vessel(BaseImageDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, data_fraction=None, min_area=None,
        ...
        self.vessel_set = Vessel(self.anno_path)

        self.cats = self.vessel_set.cats
    ...
    def _get_image_list(self, min_area=None):
        ann_list = list(self.vessel_set.anns.keys())
        image_list = [a for a in ann_list if self.vessel_set.anns[a]['iscrowd'] == 0]

        if min_area is not None:
            image_list = [a for a in image_list if self.vessel_set.anns[a]['area'] > min_area]
    ...
    def _build_im_per_class(self):
        ...
        for i, im in enumerate(self.image_list):
            class_name = self.cats[self.vessel_set.anns[im]['category_id']]['name']
    ...
    def get_image_info(self, im_id):
        ...
        mask = torch.Tensor(self.vessel_set.annToMask(anno))
    ...
    def _get_anno(self, im_id):
        anno = self.vessel_set.anns[self.image_list[im_id]]

        return anno

    def _get_image(self, im_id):
        path = self.vessel_set.loadImgs([self.vessel_set.anns[self.image_list[im_id]]['image_id']])[0]['file_name']
        img = self.image_loader(os.path.join(self.img_pth, path))
        return img

    def get_meta_info(self, im_id):
        try:
            cat_dict_current = self.cats[self.vessel_set.anns[self.image_list[im_id]]['category_id']]
