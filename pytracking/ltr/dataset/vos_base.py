...
class VOSMeta:
    ...
    def get_frame_names(self, seq_name):
        ...
        #print("_data"+str(self._data[seq_name]))
...
class MergedDatasetBase(BaseVideoDataset):
    ...
    def get_frames(self, sample_id, frame_ids, anno=None):
        ...
        """
        seq_name, obj_ids = self._samples[sample_id]
        boxannopath="/home/robotics-meta/Desktop/Jason/seg_dataset"
        meta = self.get_sequence_info(sample_id) if anno is None else anno
        frame_names = meta['frame_names']
        images = [self._load_image(self._jpeg_path / seq_name / (frame_names[f] + ".jpg")) for f in frame_ids]
        labels = [self._load_anno(self._anno_path / seq_name / (frame_names[f] + ".png")) for f in frame_ids]
        boxannos=[self._load_anno(self._boxpath / seq_name / (frame_names[f] + ".png")) for f in frame_ids]
        res1=[]
        for a in boxannos:
            res1.append(a[50:550,160:640  ])
                
        boxannos=res1
        
        res2=[]
        for a in labels:
            res2.append(a[50:550,160:640  ])
                
        labels=res2
          
        res3=[]
        for a in images:
            res3.append(a[50:550,160:640  ])
