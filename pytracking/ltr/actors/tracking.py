...
class DiMPActor(BaseActor):
    ...
    def __init__(self, net, objective, loss_weight=None):
        ...
        self.score_reconstuct = nn.Sequential(nn.Conv2d(512, 128, 3, padding=1), nn.ReLU(),
                           nn.Conv2d(128, 32, 3, padding=1),nn.ReLU(),
                           nn.Conv2d(32, 2, 3, padding=1),nn.ReLU()
                           )
    ...
    def __call__(self, data):
        ...
        """
        # Run network
        target_scores, iou_pred,skipInfo,template_feature= self.net(train_imgs=data['train_images'],
...
class DiMPMERGEDActor(BaseActor):
    """Actor for training the DiMP network."""
    def __init__(self, net, objective, loss_weight=None):
        super().__init__(net, objective)
        if loss_weight is None:
    ...
    def __call__(self, data):
        ...
        #print("shape of dataaaa"+str(data['train_masks'].view(6,600,800).shape))
        ...
        #                                   test_proposals=data['test_proposals'])
        '''
        rpn_pred_mask,template_feature,search_feature = self.net(train_imgs=data['train_images'],
                                           test_imgs=data['test_images'],
                                           train_bb=data['train_anno'],
                                           test_proposals=data['test_proposals'])
        '''
        ...
        torch.onnx.export(model, (data['train_images'],data['test_images'],data['train_masks'],data['test_masks'],data['train_anno']), onnx_model_path, verbose=True)
        ...
        #end of sketch stuff'''
        rpn_pred_mask,template_feature,search_feature =self.net(
        ...
        #print(str(data['train_images'].shape))
        
        
        
        
        #TODO this is only the mask loss
        ...
        def dice_loss(preds, targets, smooth=1e-6):
            return 1 - dice_coefficient(preds, targets, smooth)
        
        
        
        
        masks= data['test_masks']
        masks=masks.view(-1,288,288)
        
        #TODO WRITE BBOXES into fgrames
        
       
        
        
        rpn_pred_mask=rpn_pred_mask.view(-1,288,288)
        
        #masks=masks.resize(6,288*288)
        #masks = masks.view(-1, 288 * 288)
        #rpn_pred_mask = rpn_pred_mask.view(-1, 288 * 288)
        #rpn_loss_mask = F.soft_margin_loss(rpn_pred_mask,masks)
        #rpn_mask_loss=torch.mean(rpn_loss_mask)
        batch_size=6
        losses = []
        for i in range(batch_size):
            loss = dice_loss(rpn_pred_mask[i], masks[i])
        ...
        npvers=(data['train_images'][0,0].detach().cpu().permute(1,2,0).numpy()*255).astype(np.uint8)
        npvers = np.ascontiguousarray(npvers, dtype=np.uint8)
        
        
        x1=int(data['train_anno'][0,0,0])
        y1=int(data['train_anno'][0,0,1])
        x2=int(data['train_anno'][0,0,2])
        y2=int(data['train_anno'][0,0,3])
        color = (255, 0, 0)
        thickness = 2
        cv2.rectangle(npvers, (x1, y1), (x1+x2, y1+y2), color, thickness)
        
        
        
    #TODO enter where you want to save your files
        cv2.imwrite('./../bbox_included.png',(npvers))
        cv2.imwrite('./../test_image_recieved.png',(data['test_images'].detach().cpu().view(6,3,288,288)[0].permute(1, 2, 0).numpy()*255).astype(np.uint8))
        cv2.imwrite('./../train_image_recieved.png',(data['train_images'].detach().cpu().view(6,3,288,288)[0].permute(1, 2, 0).numpy()*255).astype(np.uint8))
        cv2.imwrite('./../output_text.png', (rpn_pred_mask.detach().cpu().view(6,288,288)[0].numpy()*255).astype(np.uint8))
        cv2.imwrite('./../test_mask_recieved.png',(data['test_masks'].detach().cpu().view(6,288,288)[0].numpy()*255).astype(np.uint8))
        cv2.imwrite('./../train_mask_recieved.png',(data['train_masks'].detach().cpu().view(6,288,288)[0].numpy()*255).astype(np.uint8))
        return loss, stats
