...
class DiMP_Merged(BaseTracker):

    multiobj_mode = 'parallel'

    def initialize_features(self):
        if not getattr(self, 'features_initialized', False):
            print("trying to construct net:"+str(self.params.net))
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        print("inside dimp_merged init")
        # Extract and transform sample
        init_backbone_feat,skipInfo = self.generate_init_samples(im)
    ...
    #TODO here the tracking of the dimp is done 
    def track(self, image, info: dict = None) -> dict:
        ...
        print("inside dimp_merged track calling extrackt backbone features")
        backbone_feat, sample_coords, im_patches,skipInfo = self.extract_backbone_features(im, self.get_centered_sample_pos(),
        ...
        print("ERROROOROROR!!!! should realy fix the hardcoded values in the model to adjust to batch size ")
        ...
        scores_raw=conv1(scores_raw.detach().cpu())
        print("raw score"+str(scores_raw.shape))
        
        seg=self.create_seg(scores_raw.cuda(),skipInfo)
        # Localize the target
        print("seg complete"+str(seg.shape))
        
        
        
        seg=seg.view(288,288)
        
        np_seg=seg.detach().cpu().numpy()
        out={'segmentation': np_seg}
        print("dimpmerged returns output"+str(out))
        return out

    def create_seg(self,scores_raw,skipInfo):
        return self.net.create_seg(scores_raw,skipInfo)
    ...
    def classify_target(self, sample_x: TensorList):
        ...
        with torch.no_grad():
            scores=self.net.classify_target(self.target_filter, sample_x)
    ...
    def extract_backbone_features(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        ...
        with torch.no_grad():
            print("attempt to call extract")
            backbone_feat,skipInfo = self.net.extract_backbone(im_patches)
        return backbone_feat, patch_coords, im_patches,skipInfo
    ...
    def init_classifier(self, init_backbone_feat):
        ...
        ksz = self.net.dimppart.classifier.filter_size
        ...
        with torch.no_grad():
            ...
            self.target_filter, _, losses = self.net.dimppart.classifier.get_filter(x, target_boxes, num_iter=num_iter,
    ...
    def _overwrite_classifier_params(self, feature_dim):
        # Overwrite some parameters in the classifier. (These are not generally changed)
        pred_module = getattr(self.net.dimppart.classifier.filter_optimizer, 'score_predictor', self.net.dimppart.classifier.filter_optimizer)
        if self.params.get('label_threshold', None) is not None:
            self.net.dimppart.classifier.filter_optimizer.label_threshold = self.params.label_threshold
        if self.params.get('label_shrink', None) is not None:
            self.net.dimppart.filter_optimizer.label_shrink = self.params.label_shrink
        if self.params.get('softmax_reg', None) is not None:
            self.net.dimppart.classifier.filter_optimizer.softmax_reg = self.params.softmax_reg
        ...
        if self.params.get('filter_init_zero', False):
            self.net.dimppart.classifier.filter_initializer = FilterInitializerZero(self.net.dimppart.classifier.filter_size, feature_dim)
