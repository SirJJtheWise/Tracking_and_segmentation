...
import time
def apply_filter_end(feat, filter, dilation_factors=None):
    ...
    '''#!!!TODO TODO TODO remember to change back from here
    mokfeat=torch.randn(3, 1024, 18, 18).cuda()
    
    mokffilt=torch.randn(512, 2, 4, 4).cuda()
    scores = F.conv2d(mokfeat, mokffilt,
                      padding=padding, groups=512)
    #!!!TODO TODO TODO to here just uncoment underneeth and coment this segmetnt'''
    scores = F.conv2d(feat.reshape(num_images, -1, feat.shape[-2], feat.shape[-1]), filter,
                      padding=padding, groups=512)
    
    output=scores.view(int(num_images*num_sequences), int(512/num_sequences), scores.shape[-2], scores.shape[-1])
