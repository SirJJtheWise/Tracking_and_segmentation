...
class DiMPnet(nn.Module):
    ...
    def __init__(self, feature_extractor, classifier, bb_regressor, classification_layer, bb_regressor_layer,batchsize):
        super().__init__()
        ...
        self.score_reconstuct = nn.Sequential(nn.Conv2d(int(512/batchsize), 128, 3, padding=1), nn.ReLU(),
                           nn.Conv2d(128, 32, 3, padding=1),nn.ReLU(),
                           nn.Conv2d(32, 1, 3, padding=1),nn.ReLU()
                           )

    #TODO: here the process is visable
    def forward(self, train_imgs, test_imgs, train_bb, test_proposals, *args, **kwargs):
        ...
        train_feat,template_feature = self.extract_backbone_features(train_imgs.reshape(-1, *train_imgs.shape[-3:]))
        test_feat,skipInfo = self.extract_backbone_features(test_imgs.reshape(-1, *test_imgs.shape[-3:]))
        ...
        for s in range(len(target_scores)):
            target_scores[s]=self.score_reconstuct(target_scores[s]).view(-1,self.batchsize,19,19)
        ...
        return target_scores, iou_pred,skipInfo,template_feature

    def forward_merged(self, train_imgs, test_imgs, train_bb, *args, **kwargs):
        """Runs the DiMP network the way it is applied during training.
        The forward function is ONLY used for training. Call the individual functions during tracking.
        args:
            train_imgs:  Train image samples (images, sequences, 3, H, W).
        ...
        target_scores = self.classifier(train_feat_clf, test_feat_clf, train_bb, *args, **kwargs)
        #for s in range(len(target_scores)):
            #print("target_scores"+str(target_scores[s].shape))
            #target_scores[s]=self.score_reconstuct(target_scores[s])
        ...
        #print ("target scores"+str(target_scores.shape))
        return target_scores, skipInfo,template_feature
    ...
    def final_conves(self,target_scores):
        
        k = nn.Conv2d(in_channels=512, out_channels=256, kernel_size=1, padding=0).cuda()
        
        ret=self.score_reconstuct(k(target_scores[0]))
...
#this is what we must import and construckt for the front part of the network
@model_constructor
def dimpnet50(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
              ...
              mask_init_factor=4.0, iou_input_dim=(256, 256), iou_inter_dim=(256, 256),
              score_act='relu', act_param=None, target_mask_act='sigmoid',batchsize=2,
              detach_length=float('Inf'), frozen_backbone_layers=()):

    # Backbone
    #TODO this is wher the resnet50 is created with the pretrained backbone we want to do this and transfer over this part with layers frozen and pretrained backbone
    backbone_net = backbones.resnet50(pretrained=backbone_pretrained, frozen_layers=frozen_backbone_layers)
    
    # Feature normalization
    norm_scale = math.sqrt(1.0 / (out_feature_dim * filter_size * filter_size))
#TODO this is where the clf is done 
    # Classifier features
    ...
    # DiMP network
    net = DiMPnet(feature_extractor=backbone_net, classifier=classifier, bb_regressor=bb_regressor,
                  classification_layer=classification_layer, bb_regressor_layer=['layer2', 'layer3'],batchsize=batchsize)
    return net
...
class DiMPnetMerged(nn.Module):
    ...
    def forward(self, train_imgs, test_imgs, train_bb, test_proposals, *args, **kwargs):
        ...
        assert train_imgs.dim() == 5 and test_imgs.dim() == 5, 'Expect 5 dimensional inputs'
        print("train images" + str(train_imgs.shape))
        print("test images" + str(test_imgs.shape))
        ...
        train_feat_clf = self.get_backbone_clf_feat(train_feat)
        test_feat_clf = self.get_backbone_clf_feat(test_feat)
        print("train_feat_clf"+str(train_feat_clf.shape))
        print("test_feat_clf"+str(test_feat_clf.shape))
...
@model_constructor
def dimpnet50Merged(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
              classification_layer='layer3', feat_stride=16, backbone_pretrained=True, clf_feat_blocks=0,
              clf_feat_norm=True, init_filter_norm=False, final_conv=True,
              out_feature_dim=512, init_gauss_sigma=1.0, num_dist_bins=5, bin_displacement=1.0,
              mask_init_factor=4.0, iou_input_dim=(256, 256), iou_inter_dim=(256, 256),
...
#TODO this is where the clf is done 
    ...
    # DiMP network
    net = DiMPnetMerged(feature_extractor=backbone_net, classifier=classifier, bb_regressor=bb_regressor,
                  classification_layer=classification_layer, bb_regressor_layer=['layer2', 'layer3'])
    return net
