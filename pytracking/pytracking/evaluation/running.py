...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    #print("save tracker has recieved output of shape"+str(output))
    ...
    print("seg path"+str(segmentation_path))
    frame_names = [os.path.splitext(os.path.basename(f))[0] for f in seq.frames]
    print("framenames"+str(frame_names))
...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, visdom_info=None):
    ...
    if debug:
        ...
        try:
            print("called a sequence run")
            print(str(tracker))
            output = tracker.run_sequence(seq, debug=debug, visdom_info=visdom_info)
        except Exception as e:
            print(e)
            return
    print("competed a sequence run")
    sys.stdout.flush()
